  ...
  const fetchDetailsOrder = async () => {
    let storageData = localStorage.getItem('accessToken');
          if (storageData && isJsonString(storageData)) {
              ...
              const res = await OrderService.getDetailsOrder(storageData,state?.id)
      ...
      <div style={{ width: '1270px', margin: '0 auto', height: '1270px'}}>
        ...
        <WrapperHeaderUser>
          <WrapperInfoUser>
            ...
            <WrapperContentInfo>
              <div className='name-info'><span>Tên người nhận: </span>{data?.shippingAddress?.fullName}</div>
          ...
          <WrapperInfoUser>
            ...
            <WrapperContentInfo>
              <div className='delivery-info'><span className='name-delivery'>{orderContant.delivery[data?.delivery]} </span>Giao hàng tiết kiệm</div>
        ...
        <WrapperStyleContent>
          ...
          {data?.orderItems?.map((order) => {
            return (
              <WrapperProduct>
                ...
                <WrapperItem>{convertPrice(order?.price)}<sup>đ</sup></WrapperItem>
                <WrapperItem>{order?.amount}</WrapperItem>
                <WrapperItem>{order?.discount ? convertPrice(order?.price * order?.discount / 100) : '0 VND'}<sup>đ</sup></WrapperItem>
          ...
          <WrapperAllPrice>
            ...
            <WrapperItem>{convertPrice(priceMemo)}<sup>đ</sup></WrapperItem>
          </WrapperAllPrice>
          <WrapperAllPrice>
            <WrapperItemLabel>Phí vận chuyển</WrapperItemLabel>
            <WrapperItem>{convertPrice(data?.shippingPrice)}<sup>đ</sup></WrapperItem>
          </WrapperAllPrice>
          <WrapperAllPrice>
            <WrapperItemLabel>Tổng cộng</WrapperItemLabel>
            <WrapperItem><WrapperItem>{convertPrice(data?.totalPrice)}<sup>đ</sup></WrapperItem></WrapperItem>
          </WrapperAllPrice>
      </WrapperStyleContent>
      </div>
    </div>
